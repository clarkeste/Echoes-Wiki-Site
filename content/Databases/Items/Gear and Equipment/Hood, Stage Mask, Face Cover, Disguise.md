---
tags:
  - Items
  - Items-Affordable
Location:
Damage:
Critical Rating:
Skill Used:
Range:
Encumbrance: 1
Qualities:
Soak:
Defense:
Special Effect: To identify a [[Character]] wearing a Hood or other face cover, another Character must make a [[Difficulty|Formidable]] (5p) [[Difficulty]] [[Skill Checks|Check]]. The covering can also be taken off to identity a Character. When worn, Encumbrance is 0.
Price: 50
Restricted: false
Unique: false
Unlisted: false
Is Armor: false
Is Weapon: false
Is Gear: true
Country:
Context:
Loyalty:
  - None
Image: "[[Images/thumbnails/external/1442333676b982eddfcbd91ba6cf271e.png]]"
Consumable: false
---
Various pieces of clothing are used to mask an individual’s identity, mostly by hiding their most distinctive trait—their face. Stage masks, while originally used by actors playing out the stories of playwrights or in [[Goddesses and the Gods|religious]] ceremonies, can also be used for hiding one’s identity. They come in all shapes in sizes—some replicating the face of a character, or some acting as a veil. Hoods, on the other hand, are simple pieces of clothing, but quite effective at masking one’s appearance.

If a [[Character]] is a [[Zora]] or from another People that has distinctive body-marking traits, this Item acts not only as a mask, but also a disguise to these traits. For a Zora, for instance, it may include the requisite body paint, or a medical concoction, to change the color of their scales for a short period. This may need to be refilled, and has an initial amount of 2 uses.
![](https://i.imgur.com/Lvi1uPo.png)
[[Encumbrance|Encumbrance]]: 1
Effect: To identify a [[Character]] wearing a Hood or other face cover, another Character must make a [[Difficulty|Formidable]] (5p) [[Difficulty]] [[Skill Checks|Check]]. The covering can also be taken off to identity a Character. When worn, Encumbrance is 0.

Price: 50 [[Rupees]]
[[Affiliations|Affiliation]]: None