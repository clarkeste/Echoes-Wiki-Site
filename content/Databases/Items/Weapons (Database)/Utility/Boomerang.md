---
tags:
  - Items
  - Items-Affordable
Location:
Damage: 0
Critical Rating: 5
Skill Used:
  - Melee (Light)
  - Ranged
Range: Medium
Encumbrance: 0
Qualities:
  - Concussive 1
  - Disorient 2
  - Stun Damage
  - Flammable
  - Concussive
  - Disorient
Soak:
Defense:
Special Effect: May be thrown with a [[Range Bands|Range]] of [[Medium Range|Medium]]. If thrown, the [[Ranged]] [[Skills|Skill]] is used, and the Boomerang returns to its owner at the end of the [[Attack Check|Check]]. Two [[Threat]] or a [[Despair]] can cause it to become intercepted, thrown off-course by a collision, or otherwise not make it back to its owner.
Price: 250
Unique: false
Unlisted: false
Is Armor: false
Is Weapon: true
Is Gear: false
Country:
Context:
Loyalty:
  - None
Image: "[[Images/thumbnails/external/ae65e2232f54d10238b49f784d98e313.png]]"
Weapon Type:
  - Misc
Consumable: false
---
Boomerangs, while often times just toys for children, can also be useful tools and can stun targets. They often return to their throwers.
![|494x480](https://i.imgur.com/RgiKCFk.png)
[[Damage|Damage]]: +0
[[Skills|Skill]]: [[Melee-Light|Melee (Light)]]
[[Critical Rating]]: 5
[[Range Bands|Range]]: [[Engaged Range|Engaged]]
[[Encumbrance|Encumbrance]]: 0

[[Concussive (Active) (Quality)|Concussive]] 1
[[Disorient (Active) (Quality)|Disorient]] 2
[[Stun Damage (Active) (Quality)|Stun Damage]]
[[Flammable (Passive) (Quality)|Flammable]]
Special: May be thrown with a [[Range Bands|Range]] of [[Medium Range|Medium]]. If thrown, the [[Ranged]] [[Skills|Skill]] is used, and the Boomerang returns to its owner at the end of the [[Attack Check|Check]]. Two [[Threat]] or a [[Despair]] can cause it to become intercepted, thrown off-course by a collision, or otherwise not make it back to its owner.

Price: 250 [[Rupees|Rupees]]
[[Affiliations|Affiliation]]: None